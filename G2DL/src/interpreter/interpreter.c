...
typedef struct EvaluatedArgumentNode {
    RuntimeValue value;
    struct EvaluatedArgumentNode *next;
} EvaluatedArgumentNode;

// Implementação da função principal do interpretador
RuntimeValue execute_ast(AstNode *node)
{
    void free_evaluated_argument_list(EvaluatedArgumentNode *list);
    if (!node)
    {
    ...
    {
        IdentifierNode *id_node = (IdentifierNode*)assign_node->target;
        insert_symbol(id_node->name, expr_val);
    ...
    case NODE_TYPE_BINARY_OP:
    {
            ...
            (left_val.type == TYPE_STRING || right_val.type == TYPE_STRING))
        {
            if (left_val.type == TYPE_STRING && right_val.type == TYPE_STRING)
            {
                ...
                if (left_val.data.strVal)
                    free(left_val.data.strVal);
                if (right_val.data.strVal)
                    free(right_val.data.strVal);
            ...
            else
            {
                ...
                if (left_val.type == TYPE_STRING && left_val.data.strVal)
                    free(left_val.data.strVal);
                if (right_val.type == TYPE_STRING && right_val.data.strVal)
                    free(right_val.data.strVal);
                exit(EXIT_FAILURE);
            }
        }
        ...
        case OP_DIVIDE:
            ...
            return (RuntimeValue){.type = TYPE_FLOAT, .data.floatVal = val1 / val2};
        case OP_MOD:
        { // Módulo opera apenas em inteiros
            int i_val1 = (int)val1;
            int i_val2 = (int)val2;
            if (i_val2 == 0)
            {
        ...
        case OP_POWER:
            return (RuntimeValue){.type = TYPE_FLOAT, .data.floatVal = pow(val1, val2)};

        // Operações de comparação (retornam INT como booleano)
        case OP_EQUAL:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = (val1 == val2)};
        case OP_NOT_EQUAL:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = (val1 != val2)};
        case OP_LESS_THAN:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = (val1 < val2)};
        ...
        case OP_AND:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = ((int)val1 && (int)val2)};
        case OP_OR:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = ((int)val1 || (int)val2)};
    ...
    case NODE_TYPE_UNARY_OP:
    {
        ...
        else
        {
            ...
            if (operand_val.type == TYPE_STRING && operand_val.data.strVal)
                free(operand_val.data.strVal);
            exit(EXIT_FAILURE);
        }
        ...
        {
        case OP_NOT:
            return (RuntimeValue){.type = TYPE_INT, .data.intVal = !val};
        default:
    ...
    case NODE_TYPE_BLOCK:
    {
        ...
        for(int i = 0; i < block_node->statements->count; i++)
        {
            execute_ast(block_node->statements->nodes[i]);
    ...
    case NODE_TYPE_WHILE_LOOP:
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                if (cond_val.type == TYPE_STRING && cond_val.data.strVal)
                    free(cond_val.data.strVal);
                exit(EXIT_FAILURE);
            }
            ...
            if (body_result.type == TYPE_BREAK)
            { // Supondo que você define TYPE_BREAK em types.h
                if (body_result.type == TYPE_STRING && body_result.data.strVal)
                    free(body_result.data.strVal);
                break;
            }
    ...
    case NODE_TYPE_IF_STATEMENT:
    {
        ...
        if (condition_true)
        {
            result = execute_ast(if_node->then_branch);
        }
        else if (if_node->else_branch)
        {
                result = execute_ast(if_node->else_branch);
        }
        ...
        if (result.type == TYPE_STRING && result.data.strVal)
            free(result.data.strVal);
        return (RuntimeValue){.type = TYPE_VOID};
    }
    ...
    case NODE_TYPE_PRINTF_STATEMENT:
    {
        ...
        while (current_arg_ast_node != NULL)
        {
            ...
            while (current_arg_ast_node != NULL)
            {
                ...
                RuntimeValue evaluated_value = execute_ast(current_arg_ast_node->node);

                // Copia a string se necessário
                if (evaluated_value.type == TYPE_STRING && evaluated_value.data.strVal)
                {
                    evaluated_value.data.strVal = strdup(evaluated_value.data.strVal);
                }

                // Cria um ArgumentNode temporário só com o valor
                new_runtime_arg->node = NULL;                                // Não precisa manter AST aqui, só valores
                *((RuntimeValue *)&new_runtime_arg->node) = evaluated_value; // Hack rápido se quiser, mas ideal é criar nova struct p/ argumentos avaliados
        ...
        free_evaluated_argument_list(runtime_args_list_head);
    ...
    case NODE_TYPE_PRINTF_ID:
    {
        ...
        if (sym != NULL)
        {
            ...
            EvaluatedArgumentNode *arg_list_temp = (EvaluatedArgumentNode*)malloc(sizeof(EvaluatedArgumentNode));
            if (!arg_list_temp)
            {
                perror("malloc");
                exit(EXIT_FAILURE);
...
void my_printf_runtime(char *format_string, ArgumentNode *args_list) {
    ...
    while (*p) {
        if (*p == '%' && *(p + 1)) {
            ...
            switch (*p) {
                case 'd':
                    printf("%d", ((RuntimeValue*)&current_arg->node)->data.intVal);
                    break;
                case 'f':
                    printf("%f", ((RuntimeValue*)&current_arg->node)->data.floatVal);
                    break;
                case 's':
                    printf("%s", ((RuntimeValue*)&current_arg->node)->data.strVal);
                    break;
                default:
...
void free_evaluated_argument_list(ArgumentNode *list) {
    while (list != NULL) {
        ArgumentNode *next = list->next;
        
        // Interpreta node como RuntimeValue se você usou o "hack" temporário
        RuntimeValue *val = (RuntimeValue*)&list->node;
        if (val->type == TYPE_STRING && val->data.strVal) {
